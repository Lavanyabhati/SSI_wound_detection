...
class JrDoctorUpdateForm(forms.Form):
    name = forms.CharField(max_length=100, label="Employee Name", required=True)
    email = forms.EmailField(max_length=100, label="Employee Name", required=True)
    gender = forms.ChoiceField(choices=[('Male', 'Male'), ('Female', 'Female'), ('Other', 'Other')], label="Employee Name", required=True)
    department = forms.ChoiceField(choices=DEPARTMENT_CHOICES, label="Employee Department", required=True)
...
class PostOpDayUpdateForm(forms.Form):
    date_of_procedure = forms.DateField(label='Date of Procedure', widget=forms.DateInput(attrs={"type": "date"}))
    name_of_procedure = forms.ChoiceField(label='Name of Procedure', choices=PROCEDURE_NAME_CHOICES)
    ...
    any_other = forms.CharField(label='Any other (specify below)', required=False, widget=forms.Textarea(attrs={"placeholder": "Additional details"}))
