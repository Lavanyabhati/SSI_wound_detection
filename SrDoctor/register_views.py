...
class SrDoctor:
    def __init__(self):
        ...
        self.db_sr_doctor = DBOperation(COLLECTION_SR_DOCTOR)
    ...
    def _find(self, LOG_PREFIX, phone_number):
        success = False
        ACTION = "SrDoctor._find()"
        try:
            profile_user = self.db_sr_doctor._find_one(
                filter={
    ...
    def _register_sr_doctor(self, LOG_PREFIX, data):
        try:
            ...
            register_sr_doctor = self.db_sr_doctor._insert(data=data_dict)
            print("Senior Doctor Data saved successfully!")
            return True if register_sr_doctor.inserted_id else False
    ...
    def _add_sr_doctor(self, LOG_PREFIX ,data):
        try:
            employee_id = data.get('employee_id')
            phone_number = data.get('phone_number')
            name = data.get('name', '')
            ...
            register_sr_doctor = self.db_sr_doctor._insert(upsert_filter=filter_data, data=data_dict)
            print("Senior Doctor Data saved successfully!")
            return True if register_sr_doctor.inserted_id else False
    ...
    def _update_sr_doctor(self ,LOG_PREFIX, data):
        try:
            employee_id = data.get('employee_id')
            phone_number = data.get('phone_number')
            name = data.get('name')
            ...
            update_sr_doctor = self.db_sr_doctor._update(filter_q=filter_data, update_data=data_dict, upsert=True)

            if update_sr_doctor.modified_count > 0:
                print("Senior Doctor profile updated successfully!")
                return True
            else:
                return False
