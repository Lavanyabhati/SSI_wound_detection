...
def update_jr_doctor_details(request, *args, **kwargs):
    ...
    try:
        ...
        log.info(f'{LOG_PREFIX}, "JuniorDoctorUpdateResult": {jr_doctor_details_update}')
...
@csrf_exempt
@require_http_methods(["POST"])
def add_jrDoctor_details(request, *args, **kwargs):
    ...
    try:
        decoded_body = json.loads(request.body.decode())
        form = JrDoctorForm(decoded_body)
        ...
        insert_jr_doctor = cls_nurse._register_jr_doctor(LOG_PREFIX, data=data_dict)
        log.info(f"INSERT JR DOCTOR DETAILS : {insert_jr_doctor}")

        if insert_jr_doctor:
            return JsonResponse(
                {"status": "SUCCESS", "statuscode": 200, "msg": "Junior Doctor details added successfully!", "employee_id": employee_id}
            )
        else:
            return JsonResponse({"status": "FAILURE", "statuscode": 500, "msg": "Failed to add Junior Doctor details!"})
