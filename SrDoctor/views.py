...
def add_srDoctor_details(request, *args, **kwargs):
    ...
    try:
        ...
        insert_sr_doctor = cls_nurse._register_sr_doctor(LOG_PREFIX, data=data_dict)
        log.info(f"INSERT JR DOCTOR DETAILS : {insert_sr_doctor}")

        if insert_sr_doctor:
            return JsonResponse(
                {"status": "SUCCESS", "statuscode": 200, "msg": "Senior Doctor details added successfully!", "employee_id": employee_id}
            )
        else:
